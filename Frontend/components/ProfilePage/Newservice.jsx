...
class Newservice extends Component {
    ...
    this.setState(this.props.service, () => {
      ...
      if (this.state.dprice && this.state.dprice) {
        let discount = Math.floor(
          ((this.state.price - this.state.dprice) / this.state.price) * 100
        );
      ...
      <div className="scard">
        ...
        <div className="input-group">
          ...
          <div className="input-group-area">
            <input
              ...
              onChange={this.handleChangedPrice}
              placeholder="optional - The more discount you put, you have higher chance to be seen in website"
              ref={"4"}
            />{" "}
          </div>
        </div>
        {this.state.dprice && this.state.price ? (
          <span className="input-group-notification">
            {" "}
            "Discount will be equavalent to{" "}
            {Math.floor(
              ((this.state.price - this.state.dprice) / this.state.price) * 100
            )}
            %"{" "}
          ...
          <div className="input-group-area">
            <div className="input-group-date">
              <input
                ...
                placeholder="Number of days that discount is valid"
                ref={"5"}
              />
            </div>
            <div className="input-group-date">
              <input
                ...
                onChange={this.handleChangedExpiry}
                tabIndex={"-1"}
              />
            </div>
          </div>
        ...
        <div className="input-group">
          <div className="input-group-title">
            {!this.state.saved ? "Coupon Image" : "Select Image"}
          </div>
          <div className="input-group-area">
            {!this.state.saved ? (
              <div>
                <img
                  ...
                  key={this.state.imageurl.toString()}
                ...
                {this.props.images.map(function(item, index) {
                  return (
                    <img
                      ...
                      ref={"6"}
                      className={
                        item === this.state.imageurl
                          ? "serviceImageSelected"
                          : "serviceImage"
                      }
                      onClick={() => {
                        this.handleClickImage(item);
                      }}
                      src={item}
                      key={item}
                      alt={item}
  ...
  handleChangeDescription(event) {
    this.setState({ description: event.target.value, changed: true }, () =>
  ...
  handleChangeSubCategory(event) {
    this.setState({ subcategory: event.target.value, changed: true }, () =>
      this.serviceValidation()
    );
  }
  ...
  handleChangedDate(event) {
    ...
    this.setState(
      { expiry: newDate, days: event.target.value, changed: true },
      () => this.serviceValidation()
    );
  }
  ...
  handleAddClick() {
    ...
    if (this.state.validated === true) {
      ...
      NotificationManager.error(
        "Fill the required and select a relevant image form first"
      );
    }
  }
  ...
  serviceValidation() {
    ...
    if (this.state.subcategory === "") {
      this.setState({
        subcategoryAlert: "Select Sub-Category",
        validated: false
      });
    } else {
    ...
    if (this.state.dprice === "") {
      ...
      if (
        ...
        this.setState({
          dpriceAlert: "Discounted price can not be more that price"
        });
      else this.setState({ dpriceAlert: null });
    }
    if (this.state.days === "") {
      this.setState({
        daysAlert: "Set Days to Expiry of Discount",
        validated: false
      });
    } else {
    ...
    if (this.state.description === "") {
      this.setState({
        descriptionAlert: "Provide Description",
        validated: false
      });
    } else if (this.state.description.length < 25) {
      this.setState({
        descriptionAlert: "Description is too short",
        validated: false
      });
    } else {
