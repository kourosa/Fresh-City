...
class Service extends Component {
  ...
  componentDidMount() {
    ...
    if (!token) {
      ...
      if (profile.type === "0") {
        ...
        this.API.getCompanyServices(profile.id)
          .then(res => {
            if (res.data.message === "Cannot catch items.") {
              console.log("No Item :", res.data.message);
            } else this.setState({ services: res.data.items });
          })
          .then(z => {
    ...
    return (
      <div>
        <div className="sidebar">
          ...
          <Link className="topright" onClick={this.handleSignout} to="#Update">
                ...
                {this.state.services.map(function(item, index) {
                  return (
                    <Newservice
                      ...
                      onDeleteClick={this.handleRemoveSerive}
                      onUpdateClick={this.handleUpdateSerive}
                      onAddClick={this.handleSaveSerive}
                    />
                  );
                }, this)}
              </div>
  ...
  handleAddSerive() {
    if (this.state.imagesurl.length === 0)
      NotificationManager.error(
        "You should have at least one image for your profile before add service"
      );
  ...
  handleRemoveSerive(index) {
    ...
    if (this.state.removeService === false) {
      ...
      this.API.removeCompanyService(index.itemid).then(res => {
        if (res.message === "Service or product was removed") {
          let filteredArray = this.state.services.filter(
            item => item.itemid !== index.itemid,
            this
          );
          ...
          NotificationManager.error(
            "Unable to delete the item, try later",
            null,
            1000
          );
        }
  ...
  handleUpdateSerive(index) {
    if (index.changed) {
      this.API.updateCompanyService(index).then(res => {
        if (res.message === "Service was updated")
          NotificationManager.info("Item is Updated");
        else NotificationManager.error("Unable to Update the item");
      });
    } else {
  ...
  handleSaveSerive(service) {
    if (parseInt(service.dprice) >= parseInt(service.price)) {
      ...
      this.API.addCompanyService(fullService).then(res => {
